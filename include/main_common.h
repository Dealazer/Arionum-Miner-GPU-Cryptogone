...
std::unique_ptr<IAroNonceProvider> s_nonceProvider;
std::unique_ptr<IAroResultsProcessor> s_resultsProcessor;

template <class CONTEXT, class MINING_DEVICE, class MINER>
void spawnMiners(OpenCLArguments &args, vector<AroMiner *> &miners, 
    Stats* stats, Updater *updater, MinerSettings& settings) {
    ...
    static std::vector<std::unique_ptr<MINING_DEVICE>> s_MiningDevices;
    s_MiningDevices.clear();
...
int processMinersResults(Stats *stats) {
    int nIdle = 0;
    for (int i = 0; i < s_miners.size(); i++) {
        if (s_minerIdle[i]) {
            nIdle++;
        ...
        if (s_miners[i]->resultsReady()) {
            ...
            s_minerStartT[i] = high_resolution_clock::now();
#endif
            auto result = s_miners[i]->processResults();
...
int miningLoop(Stats *stats) {
    while (true) {
        ...
        int nIdle = processMinersResults(stats);
        if (nIdle == 0) {
...
int run(const char *const *argv) {
    ...
    if (testMode()) {
        s_nonceProvider.reset(new AroNonceProviderTestMode(*stats));
        s_resultsProcessor.reset(new AroResultsProcessorTestMode());
    }
    else {
        ...
        s_nonceProvider.reset(new AroNonceProviderPool(*s_pUpdater));
        s_resultsProcessor.reset(new AroResultsProcessorPool(settings, *stats));
    }

    spawnMiners<CONTEXT, DEVICE, MINER>
        (args, s_miners, stats, s_pUpdater, settings);
