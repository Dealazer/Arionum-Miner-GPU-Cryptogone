...
const double DEFAULT_MAX_TASK_MEM = 0; // 0 <-> auto
...
struct OpenCLArguments {
    ...
    std::vector<double> maxTaskMemPerDeviceList = { DEFAULT_MAX_TASK_MEM };
    ...
    std::vector<const CommandLineOption<OpenCLArguments> *> options{
        ...
        new ArgumentOption<OpenCLArguments>(
            [](OpenCLArguments &state, const string maxMemList) {
                if (!parseFloatList(maxMemList, state.maxTaskMemPerDeviceList)) {
