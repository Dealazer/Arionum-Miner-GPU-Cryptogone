...
const uint32_t DEFAULT_MAX_TASK_MEM = 0; // 0 <-> auto
...
struct OpenCLArguments {
    ...
    std::vector<uint32_t> maxTaskMemPerDeviceList = { DEFAULT_MAX_TASK_MEM };
...
void spawnMiners(OpenCLArguments &args, vector<Miner *> &miners, Stats* stats, Updater *updater, MinerSettings& settings) {
    ...
    for (const auto &it : args.deviceIndexList) {
        ...
        size_t nThreads = args.nTasksPerDeviceList.back();
...
CommandLineParser<OpenCLArguments> buildCmdLineParser() {
    ...
    std::vector<const CommandLineOption<OpenCLArguments> *> options{
        ...
        new ArgumentOption<OpenCLArguments>(
            [](OpenCLArguments &state, const string maxMemList) {
                if (!parseUInt32List(maxMemList, state.maxTaskMemPerDeviceList)) {
                    cout << "Error parsing -m parameter, will use default" << endl;
                    state.maxTaskMemPerDeviceList = { DEFAULT_MAX_TASK_MEM };
                }
        }, "max-mem-per-task", 'm', "max GPU mem used per task, in GB, examples: -b 4 / -7.5,3.5", 
            std::to_string(DEFAULT_MAX_TASK_MEM), "maxMemPerTask"),
