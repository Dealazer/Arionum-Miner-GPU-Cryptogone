...
const string DEFAULT_CPU_BLOCKS_KERNEL = "local_state";
...
CommandLineParser<OpenCLArguments> buildCmdLineParser() {
    ...
    std::vector<const CommandLineOption<OpenCLArguments> *> options{
        ...
        new ArgumentOption<OpenCLArguments>(
            [](OpenCLArguments &state, const string cpuBlocksKernel) { 
                if (cpuBlocksKernel == "shuffle")
                    state.cpuBlocksOptimizationMode = PRECOMPUTE_SHUFFLE;
                else if (cpuBlocksKernel == "local_state")
                    state.cpuBlocksOptimizationMode = PRECOMPUTE_LOCAL_STATE;
                else {
                    cout << "Invalid kernel name: " << cpuBlocksKernel << endl;
                    exit(1);
                }
            },
            "cpu-blocks-kernel", 'k', "kernel for cpu blocks, (shuffle or local_state)",
            DEFAULT_CPU_BLOCKS_KERNEL, "kernel_name"),
